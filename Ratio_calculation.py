...
def calculate_ratio(input_csv_path, output_csv_path, column_pairs):
    ...
    for column1, column2 in column_pairs:
        ...
        ratio_column_name = f"ratio_{column1}_{column2}"
        df[ratio_column_name] = df[column1] / df[column2]
        
        # Replace inf values with NaN
        df.replace([np.inf, -np.inf], np.nan, inplace=True)
        
        # Drop rows with NaN values in the specified columns or their ratio
        df.dropna(subset=[column1, column2, ratio_column_name], inplace=True)
